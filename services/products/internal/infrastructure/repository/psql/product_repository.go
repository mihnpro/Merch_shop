...
type productRow struct {
	...
	PhotoKey    sql.NullString `db:"photo_key"`
...
func (r *productRow) toModel() *model.Product {
	...
	return &model.Product{
		...
		Sizes:     sizes,
		PhotoKey:  vo.NewPhotoKeyFromStored(r.PhotoKey.String),
		Active:    r.Active,
		Version:   r.Version,
...
const selectProductColumns = `
	p.id, p.name, p.description, p.price_points, p.sizes, p.photo_key,
...
const insertProduct = `
	INSERT INTO products (name, description, price_points, category_id, sizes, photo_key)
	VALUES ($1, $2, $3, $4, $5, $6)
...
const updateProduct = `
	...
	SET name = $1, description = $2, price_points = $3, category_id = $4,
	    sizes = $5, photo_key = $6, active = $7, version = version + 1, updated_at = NOW()
