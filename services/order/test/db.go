...
func trackOrder(id string)    { track(&createdOrders, id) }
...
func cleanup(dbs map[string]*sqlx.DB) error {
	...
	orders := append([]string(nil), createdOrders...)
	trackMu.Unlock()
	...
	if order := dbs["order"]; order != nil && len(orders) > 0 {
		exec(order, orders, `DELETE FROM order_items WHERE order_id = ANY($1)`)
		exec(order, orders, `DELETE FROM outbox WHERE payload->>'order_id' = ANY($1)`)
		exec(order, orders, `DELETE FROM orders WHERE id = ANY($1)`)
	}
	if inv := dbs["inventory"]; inv != nil && len(products) > 0 {
	...
	if user := dbs["user"]; user != nil && len(users) > 0 {
